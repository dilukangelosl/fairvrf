...
describe("FairVRFService", () => {
    ...
    describe("Integration Testing", () => {
        it("Should integrate all components correctly", async () => {
            ...
            const finalResult = await service.simulateHandleRequest(finalLog, service.getCurrentAnchor());
            ...
            expect(workflow.every(step => step.success !== false &&
                step.newAnchor !== false)).to.be.true;
        });
    });
});
...
class MockFairVRFService {
    ...
    startTime;
    metrics;
    constructor(chainPath, rotationStrategy) {
        this.chainPath = chainPath;
        this.rotationStrategy = rotationStrategy;
        // Initialize components
        this.chainManager = new TestChainManager(this.rotationStrategy || { enabled: false }, this.chainPath);
    ...
    async simulateHandleRequest(log, currentAnchor, delay = 0) {
        const startTime = Date.now();
        try {
    ...
    setSimulateNetworkError(simulate) {
...
class TestChainManager {
    ...
    getChainStats() {
        ...
        const shouldRotate = this.rotationStrategy?.enabled && (utilizationPercentage >= (this.rotationStrategy.thresholdPercentage || 80) ||
