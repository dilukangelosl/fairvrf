...
export class ChainManager {
  ...
  constructor(rotationStrategy?: Partial<RotationStrategy>) {
    ...
    console.log(
      `Rotation strategy: ${
        this.rotationStrategy.enabled ? "ENABLED" : "DISABLED"
      }`
    );
    if (this.rotationStrategy.enabled) {
      console.log(
        `  - Threshold: ${this.rotationStrategy.thresholdPercentage}% utilization`
      );
      console.log(
        `  - Min remaining: ${this.rotationStrategy.minRemainingSeeds} seeds`
      );
      console.log(
        `  - Auto-generate: ${
          this.rotationStrategy.autoGenerateNewChain ? "YES" : "NO"
        }`
      );
    }
  }
  ...
  private loadChain(): void {
    if (!fs.existsSync(this.chainPath)) {
      throw new Error(
        `Chain DB not found at ${this.chainPath}. Run generate-chain.ts first.`
      );
    }
   ...
   */
  public getNextSeed(currentAnchor: string): string {
    const normalizedAnchor = currentAnchor.toLowerCase();
    const index = this.chain.findIndex(
      (seed) => seed.toLowerCase() === normalizedAnchor
    );

    if (index === -1) {
      throw new Error(
        `Current anchor ${currentAnchor} not found in local chain DB. Sync issue?`
      );
    }
   ...
   */
  public getChainStats(): ChainStats {
    ...
    const utilizationPercentage =
      currentIndex >= 0 ? ((currentIndex + 1) / totalSeeds) * 100 : 0;

    const shouldRotate =
      this.rotationStrategy.enabled &&
   ...
   */
  private handleRotationNeeded(stats: ChainStats): void {
    console.warn(
      `ROTATION NEEDED! Chain utilization: ${stats.utilizationPercentage.toFixed(
        1
      )}%`
    );
    console.warn(
      `   Remaining seeds: ${stats.remainingSeeds}/${stats.totalSeeds}`
    );
  ...
  private generateNewChain(chainLength: number = 1000): string {
    ...
    const backupPath = this.chainPath.replace(
      ".json",
   ...
   */
  public verifySeed(seed: string, expectedHash: string): boolean {
    return keccak256(seed as `0x${string}`).toLowerCase() === expectedHash.toLowerCase();
  }
  ...
  public getHealthMetrics(): {
    ...
    if (stats.utilizationPercentage >= 90) {
      ...
      stats.utilizationPercentage >= this.rotationStrategy.thresholdPercentage
    ) {
      status = "warning";
   ...
   */
  public getCurrentAnchor(): string {
    return this.chain[0];
  }
   ...
   */
  public getChainSegment(start: number, end: number): string[] {
